...
void run_planning(double angle_x,double angle_y); // 规划主体函数
void circle();
void circle2();
void circle3();
void circle4();
...
void circle2()
{
                    ...
                    {
                          local_pos_pub.publish(aim);
                if( (abs(pos_drone_fcu[0] - aim.pose.position.x) < 0.2) && (abs(pos_drone_fcu[1] - aim.pose.position.y) < 0.2) && (abs(pos_drone_fcu[2] - aim.pose.position.z) < 0.2))
                        {
                            circle_flag=0;
                            count_ring1=0;
                        }
...
void circle3()
{
                    ...
                    else if(count_ring1 == 6)
                    {
                        ...
                        goal.pose.position.x = pos_drone_fcu[0]+4.0;
                        goal.pose.position.y = pos_drone_fcu[1];      
                        goal.pose.position.z = 1.8;
                        traj_goal_pub.publish(goal);
...
void circle5()
{
                    ...
                    else if(count_ring1 == 6)
                    {
                        ...
                        //ring_point1 equal 0
                        ring_before = Eigen::Vector3d(pos_drone_fcu[0]-ring_point1_temp[1], 
                                                    pos_drone_fcu[1]+ring_point1_temp[0], 
                                                    pos_drone_fcu[2]+ring_point1_temp[2]);
                        ring_after = Eigen::Vector3d(pos_drone_fcu[0]-ring_point2_temp[1], 
                                                    pos_drone_fcu[1]+ring_point2_temp[0], 
                                                    pos_drone_fcu[2]+ring_point2_temp[2]);      
...
int main(int argc, char **argv)
{
    ...
    nh.param("single_offboard/yaw_rate_max", D_YAW_MAX, 0.4);
...
//        if(cmdd!=4) local_pos_pub.publish(aim);
        ...
        if(cmdd==9)
        {
            ...
            if( (abs(pos_drone_fcu[0] - aim.pose.position.x) < 0.2) && (abs(pos_drone_fcu[1] - aim.pose.position.y) < 0.2) && (abs(pos_drone_fcu[2] - aim.pose.position.z) < 0.2) && btt==0)
                        {
                          btt=1;
                          bt=-bt;
                        }            
        ...
        {        
            ascup_flag.data=3;
            static int flag_712=0;
            if(flag_712==0)
            {
                static int yaw_712=0;   

                if(yaw_712==0)
                {
                    // aim.pose.position.x =  pos_drone_fcu[0]+3.7;
                    // aim.pose.position.y =  pos_drone_fcu[1];      
                    // aim.pose.position.z =  pos_drone_fcu[2];

                    //aim.pose.position.x =  pos_drone_fcu[0]+3.7;
                    aim.pose.position.x =  28.24;
                    aim.pose.position.y =  pos_drone_fcu[1];      
                    aim.pose.position.z =  pos_drone_fcu[2];
                    yaw_712=1;
                }
            ...
            {
                run_planning(0.0,1.0);
                if(abs(yawyaw-PI/2)<0.1)
                flag_712++;
        ...
        {        
            ascup_flag.data=3;
            static int flag_713=0;
            if(flag_713==0)
            {
                static int yaw_713=0;   

                if(yaw_713==0)
                {
                    aim.pose.position.x =  pos_drone_fcu[0];
                    //aim.pose.position.y =  pos_drone_fcu[1]+2.5;      
                    aim.pose.position.y =  5.53;      
                    aim.pose.position.z =  pos_drone_fcu[2];
                    yaw_713=1;
                }
      ...
      if(cmdd==714) 
        {
            static int flag_714=0;
            if(flag_714==0)
            {
                        run_planning(-1.0,0.0);

                        goal.pose.position.x = 16.1;
                        goal.pose.position.y = pos_drone_fcu[1];      
                        goal.pose.position.z = 0.9;
                        traj_goal_pub.publish(goal);

                        goal.pose.position.x = 13.1;
                        goal.pose.position.y = pos_drone_fcu[1];      
                        goal.pose.position.z = 0.9;
                        traj_goal_pub.publish(goal);
...
void run_planning(double angle_x, double angle_y)
{            
    // CAL_YAW
    double arg_    = atan2(-angle_x, angle_y) + (PI/2.0f);
    double vel_len = sqrt(pow(angle_x,2)+pow(angle_y,2));
